package ch05_interface.basic.ch08.interfaceimplements;
...
// 에러남
    ...
    /* 필기. 또한 default 키워드를 사용하면 non-static 메소드도 작성 가능하다 (JDK 1.8 (8버전) 이후 추가된 기능) */
    // 설명 우리가 기본적인 클래스에서 만들던 일반 메서드 처럼 인터페이스에서 만들기 위해서는 default 키워드를 붙여준다고 생각하면된다.
    // 해당 인터페이스를 구현하는 클래스는 오버라이드 하지 않아도, default메서드를 사용 가능하다.
